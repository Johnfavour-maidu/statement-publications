...
function LoginForm() {
  ...
  return (
    ...
    >
      ...
      {/* Card */}
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.08)] border border-white/60 p-8">
        <div className="text-center mb-6">
          <h1 className="text-[22px] font-bold text-[#1D1D1D]">Welcome Back</h1>
          <p className="text-[13px] text-gray-500 mt-1.5">Sign in to your Statement Books account</p>
        </div>

        {registered && (
          <div className="mb-4 p-3 rounded-xl bg-green-50 border border-green-200/60 text-[13px] text-green-700">
            Account created successfully! Please sign in.
          </div>
        )}

        {oauthError && (
          <div className="mb-4 p-3 rounded-xl bg-red-50 border border-red-200/60 text-[13px] text-red-700">
            This email is already associated with another sign-in method. Please use your original sign-in method.
          </div>
        )}

        {error && (
          <div className="mb-4 p-3 rounded-xl bg-red-50 border border-red-200/60 text-[13px] text-red-700 flex items-start gap-2">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input
                ...
                className="pl-10 h-11 rounded-xl border-gray-200 focus:border-[#D8B27A] focus:ring-[#D8B27A]/20 text-[13px]"
                required
              />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input
                ...
                className="pl-10 pr-10 h-11 rounded-xl border-gray-200 focus:border-[#D8B27A] focus:ring-[#D8B27A]/20 text-[13px]"
                required
              />
              <button
                type="button"
          ...
          <Button
            type="submit"
            disabled={isLoading}
            className="w-full h-11 bg-[#D8B27A] hover:bg-[#8A6A4A] text-[#1D1D1D] hover:text-white font-semibold rounded-xl transition-all duration-200 text-[13px]"
          >
            {isLoading ? (
        ...
        {showResendVerification && (
          <div className="mt-4 p-3 rounded-xl bg-blue-50 border border-blue-200/60">
            <p className="text-[13px] text-blue-700 mb-2">Need a new verification email?</p>
            <div className="flex items-center gap-2">
              <Input
                type="email"
                value={resendEmail}
        ...
        <Button
          ...
          variant="outline"
          className="w-full h-11 border-gray-200 hover:bg-gray-50 transition-all duration-200 rounded-xl text-[13px] font-medium"
        >
          {isGoogleLoading ? (
